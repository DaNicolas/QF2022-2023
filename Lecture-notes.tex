...
Hence, we obtain $W \oplus W^\perp$, and the natural induced $K$-isomorphism $V \to W \times W^\perp$ gives the required isometry $(V, q) \cong (W, q\vert_W) \perp (W^\perp, q\vert_{W^\perp})$.
...
Let $W$ be a totally isotropic subspace of $V$ of dimension $n$.
By \Cref{P:hyperbolic-plane} there is a subspace $W'$ of $V$ such that $(W', q\vert_{W'}) \cong \mbb{H}_K$, and by \Cref{P:splitting-off} this implies that $(V, q) \cong \mbb{H}_K \perp (U, q\vert_U)$ for $U=(W')^\perp$, and furthermore $(U, q\vert_U)$ is nonsingular.
Observe that $\dim(W \cap W') \leq 1$, since $(W', q\vert_{W'})$ is not totally isotropic.
By \Cref{P:dim-duality} we have that $\dim(W \cap W') + \dim(W \cap U) = \dim(W) = n$, and thus $\dim(W \cap U) \geq n-1$, whereby $(U, q\vert_U)$ contains a totally isotropic subspace of dimension $n-1$.
