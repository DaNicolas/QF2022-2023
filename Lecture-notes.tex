...
We recommend the book of Elman, Karpenko and Merkurjev \autocite{ElmanKarpenkoMerkurjev} for those who want to learn more about quadratic form theory over fields of arbitrary charcacteristic.
...
\begin{itemize}
\item $V$ is a finite-dimensionl vector space over $K$, and
\item $B : V \times V \to K$ is symmetric and bilinear, i.e.~for all $x, x', y \in V$ and $a \in K$ we have
\begin{align*}
B(x, y) &= B(y, x), \\
B(x+x',y) &= B(x, y) + B(x', y), \\
B(ax, y) &= aB(x, y).
...
For column vectors $x = [x_1 \ldots x_n]^T$ and $y = [y_1, \ldots, y_n]$ we have
...
An isomorphism of $K$-vector spaces $I : V \to V'$ is called \emph{isometry between $(V, B)$ and $(V', B')$}\index{isometry} if, for all $v, w \in V$, one has $B(v, w) = B'(I(v), I(w))$.
...
\end{displaymath}
\end{eg}
We saw that, to a quadratic form $q$, one a symmetric bilinear form $\mf{b}_q$ on the same space.
...
Then $q$ is equal to the quadratic form associated to the polar form $\frac{1}{2}\mf{b}_q$.
...
\item Given $a \in K^\times$, we say that \emph{$q$ represents $a$}\emph{representation!of an element by a form} if $\exists v \in V$ with $q = q(v)$.
We write
...
$$ f\left(\frac{X_1 + X_2}{2}, \frac{X_1 - X_2}{2}\right) = X_1^2.$$
We will often want to exclude from our study quadratic forms which have this property, which we now study in some detail.
...
Otherwise, we call the form \emph{singular}.
\end{defi}
...
Note that, if $\charac(K) = 2$, these terms have more specialised meanings.
...
and call it the \emph{orthogonal space of $M$}. We write $v^\perp$ instead of $\lbrace v \rbrace^\perp$.
...
\end{align*}
We observe that $\varphi_1$ is injective by the nonsingularity of $(V, B)$, that $\varphi_2$ is surjective, and that the image of $\varphi_1$ is precisely the kernal of $\varphi_2$ by definition of $U^\perp$.
...
Then $q \cong q_1 \perp q_2$ if and only if there are $K$-subspaces $W_1$ and $W_2$ of $V$ with $W_1 \perp W_2$ with respect to $q$, $V = W_1 \oplus W_2$, and such that $(W_i, q\vert_{W_i}) \cong (V_i, q_i)$ for $i = 1, 2$.
\end{prop}
\begin{proof}
...
Without loss of generality, we may assume that $q_i = q\vert_{W_i}$.
...
\begin{defi}
Let $a_1, \ldots, a_n \in K$. We denote by $\langle a_1, \ldots, a_n \rangle$ the quadratic form
$$ K^n \to K : (x_1, \ldots, x_n) \mapsto \sum_{i=1}^n a_ix_i^2.$$
We call such a form a \emph{diagonal form}\index{diagonal form}.
\end{defi}
Note that $\langle a_1, \ldots, a_n \rangle \cong \langle a_1 \rangle \perp \ldots \perp \langle a_n \rangle$.
...
Then $(W, q\vert_W)$ is nonsingular, and $D_K(q\vert_W) = D_K(q)$.
...
Set $U = v^\perp$.
We have $v \not\in v^\perp$ (since $\mf{b}_q(v, v) = 2d \neq 0$) and $\dim(U) = \dim(V) - 1$, hence $V = Kv \oplus U$.
