...
Let $(V, q)$ be a nonsingular quadratic space, $W$ a nonsingular subspace of $V$, $W'$ an orthogonal complement of $W$.
Then
...
Since $(V, q) \perp (V, -q)$ is nonsingular (by \Cref{P:orth-sum}) and has a totally isotropic subspace of dimension $n$, it is hyperbolic by \Cref{C:hyperbolic-form}.
\end{proof}
...
\end{thm}
\begin{proof}
By \Cref{C:diagonalisation} we may assume that $(V, q) \cong \langle a_1, \ldots, a_n \rangle$ for some $a_1, \ldots, a_n \in K$.
...
\underline{Case 1: $n=1$ and $a \neq 0$}: 
We need to show that, if $\langle a \rangle_K \perp (V_1, q_1) \cong \langle a \rangle_K \perp (V_2, q_2)$, then $(V_1, q_1) \cong (V_2, q_2)$.
Let $v = \iota(1, 0)$.
...
Furthermore, since $K(1, 0) \perp (\lbrace 0 \rbrace \times V_1)$ (in $(K \times V_1, \langle a \rangle_K \perp q_1)$) and isometries preserve orthogonality, we obtain $K(1, 0) \perp (\tau \circ \iota)(\lbrace 0 \rbrace \times V_1)$ (in $(K \times V_2, \langle a \rangle_K \perp q_2)$).
...
\underline{Case 2: $n \geq 1$, $a_1 = \ldots = a_n = 0$, and $(V_1, q_1)$ is nonsingular}:
In this case, $(V, q)$ is totally isotropic.
We have $(V \times V_1)^\perp = V^\perp \times V_1^\perp = V^\perp \times \lbrace 0 \rbrace$.
By comparing dimensions, we see that also $(V_2, q_2)$ is nonsingular, and $(V \times V_2)^\perp = V^\perp \times V_2^\perp = V^\perp \times \lbrace 0 \rbrace$.
In view of \Cref{P:decomposition-totally-isotropic} we obtain that $(V_1, q_1) \cong ((V \times V_1)/(V \times V_1)^\perp, \overline{q \perp q_1}) \cong ((V \times V_2)/(V \times V_2)^\perp, \overline{q \perp q_2}) \cong (V_2, q_2)$.

\underline{General case}:
This follows inductively from the first two cases, since by \Cref{P:decomposition-totally-isotropic} both $(V_1, q_1)$ and $(V, q)$ can be decomposed uniquely up to isometry as the orthogonal sum of an nonsingular quadratic form and a totally isotropic one.
\end{proof}
