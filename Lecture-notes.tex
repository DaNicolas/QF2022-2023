...
\end{prop}
\begin{proof}
It follows from \Cref{L:split-off-2fold-Pfister} by induction on $m$ that we can find a quadratic form $\pi_m$ and scaled $2$-fold Pfister forms $\pi_1, \ldots, \pi_{m-1}$ such that $ [(V, q)] = \sum_{i=1}^m [\pi_i] $.
Since $[(V, q)] \in I^2K$, we obtain by \Cref{P:disc-map-kernel} that $1 = \disc(V, q) = \disc(\pi_1)\cdots\disc(\pi_m) = \disc(\pi_m)$ in $K^{\times}/K^{\times 2}$.
Now it follows by exercise \ref{Exercises-L4}.\ref{ex:4-dim-Pfister} that also $\pi_m$ is a scaled $2$-fold Pfister form.
\end{proof}
